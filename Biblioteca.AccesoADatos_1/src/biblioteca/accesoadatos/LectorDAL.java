...
public class LectorDAL {
    static String obtenerCampos() {
        return "m.Id,m.Nombre,m.Apellido,m.Dui";
    ...
    private static String obtenerSelect(Lector pLector) {
        ...
        sql += (obtenerCampos() + " FROM Lector m"); // Agregar los campos de la tabla de Rol mas el FROM a la tabla Rol con su alias "r"
        return sql;
    } 
...
static int asignarDatosResultSet(Lector pLector, ResultSet pResultSet, int pIndex) throws Exception {
    ...
    public static Lector obtenerPorId(Lector pLector) throws Exception {
        ...
        if (pLector.getApellido()!= null && pLector.getApellido().trim().isEmpty() == false) {
            pUtilQuery.AgregarWhereAnd(" m.Apellido LIKE ? "); // Agregar el campo Nombre al filtro de la consulta SELECT y agregar en el WHERE o AND
            if (statement != null) {
                // Agregar el parametro del campo Nombre a la consulta SELECT de la tabla de Rol
        ...
        ()!= null && pLector.getDui().trim().isEmpty() == false) {
            pUtilQuery.AgregarWhereAnd(" m.Dui LIKE ? "); // Agregar el campo Nombre al filtro de la consulta SELECT y agregar en el WHERE o AND
            if (statement != null) {
                // Agregar el parametro del campo Nombre a la consulta SELECT de la tabla de Rol
