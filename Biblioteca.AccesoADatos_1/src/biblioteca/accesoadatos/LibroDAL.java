...
public class LibroDAL {
    ...
    static String obtenerCampos() {
        return "l.Id,l.Nombre, l.Edicion,l.IdAutor, l.IdCategoria, l.IdEditorial, l.FechaEdicion";
    }
 ...
 public static int crear(Libro pLibro) throws Exception {
        ...
        try (Connection conn = ComunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            sql = "INSERT INTO Libros(Nombre,IdAutor,IdCategoria,IdEditorial,FechaEdicion) VALUES(?,?,?,?,?)"; // Definir la consulta INSERT a la tabla de Rol utilizando el simbolo ? para enviar parametros
...
public static int modificar(Libro pLibro) throws Exception {
        ...
        String sql;
        try (Connection conn = ComunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            sql = "UPDATE Libros SET Nombre=?  ,IdAutor=? , IdCategoria=?,IdEditorial=?,FechaEdicion=? WHERE Id=?"; // Definir la consulta UPDATE a la tabla de Rol utilizando el simbolo ? para enviar parametros
            try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ComunDB
                ps.setString(1, pLibro.getNombre()); 
                ps.setString(2, pLibro.getEdicion());
    ...
    private static void obtenerDatosIncluirACE(PreparedStatement pPS, ArrayList<Libro> pLibros) throws Exception {
       ...
       static void querySelect(Libro pLibro, ComunDB.UtilQuery pUtilQuery) throws SQLException {
        ...
        if (pLibro.getFechaEdicion()!= null && pLibro.getFechaEdicion().trim().isEmpty() == false) {
            pUtilQuery.AgregarWhereAnd(" l.FechaEdicion LIKE ? "); // Agregar el campo Nombre al filtro de la consulta SELECT y agregar en el WHERE o AND
            if (statement != null) {
                // Agregar el parametro del campo Nombre a la consulta SELECT de la tabla de Rol
